...
//	}
	...
	public void testsearchBike() {
		//Test if bikeList must not be Null, so that we can iterate through the bikeList and find the bike item information
		assertNotNull("Test if bikeList is not null", bikeList);
	}
	@Test
	public void testsearchBikeParts() {
		//Test if bikePartList must not be Null, so that we can iterate through the bikePartList and find the bike part item information
